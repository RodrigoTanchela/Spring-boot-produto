...
import com.example.springboot.exceptions.InvalidJwtAuthenticationException;
...
public class JwtTokenProvider {
	...
	private String getRefreshToken(String username, List<String> roles, Date now) {
		Date validateToken = new Date((now.getTime() + validityInMilliseconds * 3));
		return JWT.create()
				.withClaim("roles", roles)
				.withIssuedAt(now)
				.withExpiresAt(validateToken)
				.withSubject(username)
				.sign(algorithm)
				.strip();
	}
	...
	private DecodedJWT decodedToken(String token) {
		...
		JWTVerifier verifer = JWT.require(alg).build();
